...
const getCartByCustomer = async (req,res) =>{
    ...
    var sql = "SELECT c.cart_id, c.quantity, p.* FROM cart c"; // c.* select all file of cart table/ p.* select all file of product table
    sql += " INNER JOIN product p ON (c.product_id = p.product_id)"; //  (sql +=) == sql = sql + " INNER JOIN product p ON c.product_id = p.product_id";
    sql += " WHERE c.customer_id = ?"; // (sql +=) == sql = sql + " WHERE c.customer_id = ?";
